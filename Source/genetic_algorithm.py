...
class GeneticAlgorithm:
  ...
  def __init__(self, estimator, parameters_ranges: dict, X_train, y_train, size_of_population: int, fitness_limit, time_limit, probability: float = 0.1, mutation_number: int = 1):
  ...
  def mean_score_population(self, population):
    return sum([self.fitness_function(chromosome) for chromosome in population]) / len(population)

  def plot_generations_scores(self):
    scores = self.get_scores()
    mean_score = self.get_mean_scores()
    fig = plt.figure(figsize = (15, 10))
    plt.plot(scores, label = 'Max score per generation')
  ...
  def fitness_function(self, chromosome): 
    self.estimator = self.estimator.set_params(**chromosome)
    return cross_val_score(self.estimator, self.X_train, self.y_train, cv = StratifiedKFold(5, shuffle = True, random_state = 0), scoring = 'precision').mean()
